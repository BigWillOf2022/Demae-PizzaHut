...
func (d *QuikOrder) StoreLookup(zipCode, address string) ([]Store, error) {
	...
	go d.sendAsyncGET(fmt.Sprintf("%s", d.apiURL, queryParams.Encode()), respChan)
...
func (d *QuikOrder) AddressLookup(zipCode, address string) (*User, error) {
	queryParams := url.Values{
		"type": {"Delivery"},
		"c":    {zipCode},
		"s":    {address},
...
func (d *QuikOrder) GetStoreInfo(storeId string) (*Store, error) {
	respChan := make(chan *http.Response)
...
func (d *QuikOrder) GetMenu(storeId string) ([]MenuCategory, error) {
	respChan := make(chan *http.Response)
...
func (d *QuikOrder) GetItemList(storeId string, menuCode string) ([]Item, error) {
	respChan := make(chan *http.Response)
	go d.sendAsyncGET(fmt.Sprintf("%s/power/store/%s/menu?lang=en&structured=true", d.apiURL, storeId), respChan)
...
func (d *QuikOrder) GetFoodPrice(storeId string, itemId string) (string, error) {
	respChan := make(chan *http.Response)
	go d.sendAsyncGET(fmt.Sprintf("%s/power/store/%s/menu?lang=en&structured=true", d.apiURL, storeId), respChan)
...
func (d *QuikOrder) GetToppings(storeId string, itemId string) ([]Topping, error) {
	respChan := make(chan *http.Response)
	go d.sendAsyncGET(fmt.Sprintf("%s/power/store/%s/menu?lang=en&structured=true", d.apiURL, storeId), respChan)
...
func (d *QuikOrder) GetSides(storeId string, itemId string) ([]Topping, error) {
...
func (d *QuikOrder) AddItem(storeId, itemId, quantity string, extraToppings []Topping) (map[string]any, error) {
	respChan := make(chan *http.Response)
	go d.sendAsyncGET(fmt.Sprintf("%s/power/store/%s/menu?lang=en&structured=true", d.apiURL, storeId), respChan)
...
func (d *QuikOrder) GetItemPrice(storeId, itemID string) (string, string, error) {
	respChan := make(chan *http.Response)
	go d.sendAsyncGET(fmt.Sprintf("%s/power/store/%s/menu?lang=en&structured=true", d.apiURL, storeId), respChan)
...
func (d *QuikOrder) GetPrice(user *User) (*Basket, error) {
...
func (d *QuikOrder) PlaceOrder(info *User) error {
	payload := map[string]any{}
	payload["Status"] = 0
	payload["Order"] = map[string]any{
		"Address": map[string]any{
		...
		"ServiceMethod":          "Delivery",
		"SourceOrganizationURI":  "pizzahut.com",
	...
	if d.country == CANADA {
		payload["Order"].(map[string]any)["SourceOrganizationURI"] = "pizzaut.ca"
	}
...
func (d *QuikOrder) sendAsyncGET(url string, rc chan *http.Response) {
	client := &http.Client{}
...
func (d *QuikOrder) sendAsyncPOST(url string, data []byte, rc chan *http.Response) {
	client := &http.Client{}
...
func (d *QuikOrder) setHeaders(req *http.Request) {
...
func (d *QuikOrder) setPostHeaders(req *http.Request) {
