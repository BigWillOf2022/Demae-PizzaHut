...
func menuList(r *Response) {
	...
	menuData, err := r.quikorder.GetMenu(r.request.URL.Query().Get("shopCode"))
	if err != nil {
...
func itemList(r *Response) {
	...
	itemData, err := r.quikorder.GetItemList(r.request.URL.Query().Get("shopCode"), r.request.URL.Query().Get("menuCode"))
	if err != nil {
		r.ReportError(err, http.StatusInternalServerError)
		return
	}
...
func itemOne(r *Response) {
	...
	price, err := r.quikorder.GetFoodPrice(r.request.URL.Query().Get("shopCode"), r.request.URL.Query().Get("itemCode"))
	if err != nil {
		r.ReportError(err, http.StatusInternalServerError)
		return
	}
