...
func basketAdd(r *Response) {
	...
	var lastBasket string
	var err error
	r.quikorder, err = quikorder.Newquikorder(r.request)
	if err != nil {
		r.ReportError(err, http.StatusInternalServerError)
		return
	}
	...
	for items, _ := range r.request.PostForm {
		if strings.Contains(items, "option") {
			...
			toppings = append(toppings, quikorder.Topping{
				Code:  code,
				Name:  "",
				Group: toppingType,
			})
	...
	// Create our basket
	basket, err := r.quikorder.AddItem(shopCode, itemCode, quantity, toppings)
	if err != nil {
		r.ReportError(err, http.StatusInternalServerError)
		return
	}
...
func basketList(r *Response) {
	...
	user, err := r.quikorder.AddressLookup(postalCode, address)
	if err != nil {
		r.ReportError(err, http.StatusInternalServerError)
		return
	}
	...
	items, err := r.quikorder.GetPrice(user)
	if err != nil {
		r.ReportError(err, http.StatusInternalServerError)
		return
	}
...
func orderDone(r *Response) {
	...
	user, err := r.quikorder.AddressLookup(r.request.PostForm.Get("member[PostNo]"), r.request.PostForm.Get("member[Address5]"))
	if err != nil {
		r.ReportError(err, http.StatusInternalServerError)
		return
	}
	...
	didError := false
	err = r.quikorder.PlaceOrder(user)
	if err != nil {
		PostDiscordWebhook(
